...
public class CrystalliteLeggingsAmethystClimbWallsProcedure {
	...
	private static void execute(@Nullable Event event, LevelAccessor world, double x, double y, double z, Entity entity) {
		if (entity == null)
			return;
		if (IsWearingArmorTagProcedure.execute(entity, "better_tools:wall_climbing_armor") && !(entity instanceof Player _plr ? _plr.getAbilities().instabuild : false)
				&& (world.getBlockState(BlockPos.containing(x + (entity.getDirection()).getStepX(), y, z + (entity.getDirection()).getStepZ())).isFaceSturdy(world,
						BlockPos.containing(x + (entity.getDirection()).getStepX(), y, z + (entity.getDirection()).getStepZ()), ((entity.getDirection()).getOpposite()))
						|| (world.getBlockState(BlockPos.containing(x + (entity.getDirection()).getStepX(), y, z + (entity.getDirection()).getStepZ())))
