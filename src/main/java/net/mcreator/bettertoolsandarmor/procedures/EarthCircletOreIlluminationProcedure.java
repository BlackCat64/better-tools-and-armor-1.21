	...
	private static void execute(@Nullable Event event, LevelAccessor world, double x, double y, double z, Entity entity) {
		...
		if ((HasCuriosItemEquippedProcedure.execute(world, entity, new ItemStack(BetterToolsModItems.EARTH_CIRCLET.get()))
				...
				|| entity instanceof LivingEntity _livEnt2 && _livEnt2.hasEffect(BetterToolsModMobEffects.ORE_VISION)) && entity.getData(BetterToolsModVariables.PLAYER_VARIABLES).crystallite_amethyst_ore_highlight_cooldown == 0) {
			{
				...
				_vars.crystallite_amethyst_ore_highlight_cooldown = Mth.nextInt(RandomSource.create(), 80, 120);
				_vars.markSyncDirty();
			}
			radius = 24;
			sx = radius * (-1);
			for (int index208 = 0; index208 < (int) (radius * 2); index208++) {
				sy = radius * (-1);
				for (int index209 = 0; index209 < (int) (radius * 2); index209++) {
					sz = radius * (-1);
					for (int index210 = 0; index210 < (int) (radius * 2); index210++) {
						if ((world.getBlockState(BlockPos.containing(x + sx, y + sy, z + sz))).is(BlockTags.create(ResourceLocation.parse("c:ores")))) {
							...
							for (Direction directioniterator : Direction.values()) {
								if (world.isEmptyBlock(BlockPos.containing(block_x + directioniterator.getStepX(), block_y + directioniterator.getStepY(), block_z + directioniterator.getStepZ()))) {
									for (int index211 = 0; index211 < Mth.nextInt(RandomSource.create(), 1, 3); index211++) {
