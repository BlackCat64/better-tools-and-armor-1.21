...
public class CrystalliteShovelEmeraldProcedureProcedure {
	...
	private static void execute(@Nullable Event event, LevelAccessor world, double x, double y, double z, BlockState blockstate, Entity entity) {
		if (entity == null)
			return;
		double y_distance = 0;
		if ((entity instanceof LivingEntity _livEnt ? _livEnt.getMainHandItem() : ItemStack.EMPTY).is(ItemTags.create(ResourceLocation.parse("better_tools:top_sand_mining_shovels")))
				&& !(entity instanceof Player _plr ? _plr.getAbilities().instabuild : false) && !entity.isShiftKeyDown()
				&& (blockstate.is(BlockTags.create(ResourceLocation.parse("minecraft:sand"))) || blockstate.is(BlockTags.create(ResourceLocation.parse("c:gravels"))))) {
			y_distance = 0;
			for (int index233 = 0; index233 < 16; index233++) {
